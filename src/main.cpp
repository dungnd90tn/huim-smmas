...
int main ( int argc, char *argv[] )
{
	...
	*output << "Pheromone Updating Parameter: " << a.get<double>("rho") << endl ;
	//*output << "T-min:" << Node::getTmin() << " - Number Nodes:"<< Node::getNumberNode()<< endl;
	*output << "Processing Ant Colony System Algorithm..." << endl;
	...
	while (!sNode->finish() && ants.getNIteration() <= maxIteration) {
		ants.runIteration();
		if (output == &cout)
			//*output << "\rIteration:\t" << ants.getNIteration() << " / " << maxIteration << "\t\t "<< "NUM of HUIs:\t" << ants.huis.size() <<"\tPheromone:\t"<< Node::printPheromone() << flush;
		...
		if (ants.getNIteration() % a.get<unsigned>("interval") == 0) {
			//*output << "\rIteration:\t" << ants.getNIteration() << " / " << maxIteration << "\t\t "<< "NUM of HUIs:\t" << ants.huis.size() <<"\tPheromone:\t"<< Node::printPheromone() << endl;
